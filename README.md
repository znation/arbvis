...
**Grid resolution (`--grid N`)** — the target *detail* resolution: the voxel cube side, a power of two in `2–16384` (default `2048`). Above a fixed coarse cap (`128`), the mandatory up-front download stays **small and constant** — a coarse `128³` fallback (≈ 8 MB) plus a sparse **octree** page structure — and the fine detail streams on demand from a brick pool as you pan/zoom (ray-guided, GigaVoxels-style, into a bounded GPU cache). So higher `--grid` costs more disk on the host but *not* a bigger initial download: `512³`, `2048³`, and `8192³` all start around ~8 MB and sharpen in place. (At or below `128`, the whole grid is small enough to download and upload once, so it does — no streaming.)

The page structure is a GigaVoxels-style N³-tree (an octree over the brick grid) that materializes only non-empty subtrees, so it is **O(occupied), independent of `N`** — the node pool is a fraction of a megabyte at `512³`, `2048³`, *and* `8192³` for typical data. Nothing in the up-front download, client RAM, or VRAM scales with the volume; only `bricks.bin` (on the host, range-streamed) grows with the actual data present.
