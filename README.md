...
Where 2D color is fully opaque, **3D uses opacity to encode density** so you can see *through* the cube to its internal structure instead of just an opaque shell. The viewer is a GPU ray-march of a bounded voxel grid: color encodes the mean byte value (the same [byte-color scheme](#byte-colors) as 2D); opacity comes from an adjustable, log-style **transfer function**. Render cost depends on the grid resolution, *not* the input size, so a multi-GB file renders as smoothly as a small one — and above a small coarse resolution the fine detail **streams on demand** (see **Grid resolution** below), so the initial download stays small however high you push the detail.
...
**Grid resolution (`--grid N`)** — the target *detail* resolution: the voxel cube side, a power of two in `2–8192` (default `2048`). Above a fixed coarse cap (`128`), the mandatory up-front download stays **small and constant** — a coarse `128³` fallback (≈ 8 MB) plus a page table — and the fine detail streams on demand from a sparse brick pool as you pan/zoom (ray-guided, GigaVoxels-style, into a bounded GPU cache). So higher `--grid` costs more disk on the host but *not* a bigger initial download: `512³` and `2048³` both start in a few-MB-to-tens-of-MB range and sharpen in place. (At or below `128`, the whole grid is small enough to download and upload once, so it does — no streaming.)

The one thing that still grows with detail is the page table (≈ `(N/8)³ · 4` bytes: ~1 MB at `512³`, ~67 MB at `2048³`, ~512 MB at `4096³`, ~4 GB at `8192³`), which the viewer fetches and holds whole. The deployed Space gzips it on the wire (it's mostly zero, so it deflates enormously — a 64 MB `2048³` table ships as ~0.1 MB), but the client still expands it in RAM, so **`--grid ≥ 4096` is only practical on the gzipped Space and may exceed browser memory** until the page table goes sparse/octree (a planned follow-up). `2048³` (the default) is comfortable everywhere.

**Volume resolution (`--volume-res N`)** — advanced override for the coarse/detail split: build the dense coarse grid at `--grid` and the streamed sparse brick pool at `--volume-res` (power of two, above `--grid`) instead of deriving the split automatically. `0` (default) derives it from `--grid`.

> **Self-hosting note:** the streamed brick pool is fetched with HTTP Range requests, so for efficient streaming serve the bundle from a host that honors `Range` (the deployed Space and most CDNs/static hosts do; `npx serve` does). The viewer still works on a host that ignores `Range` (e.g. `python3 -m http.server`, which replies `200` with the whole file) — it falls back to fetching `bricks.bin` whole — but that defeats the streaming, so prefer a `Range`-capable host for large `--grid`. For the smallest transfer, also enable gzip for `volume.bin`/`pagetable.bin`/`meta.json` — the deployed Space does this for you.
