...
/// Build a diff buffer at the original file's byte layout.
///
/// For each tensor present in both files with matching dtype+shape: decode elements,
/// compute abs(a-b), normalize globally to [0, 255], and write u8 values at the
/// original file's byte positions. Unmatched tensors in the original are filled with 0.
/// The header region is always 0 (no diff).
pub fn build_diff_buffer(original: &[u8], modified: &[u8]) -> anyhow::Result<Vec<u8>> {
    let (orig_tensors, _orig_header_end) = parse_header(original)?;
    let (mod_tensors, _mod_header_end) = parse_header(modified)?;

    // Build a lookup from tensor name to modified tensor metadata.
    let mod_map: std::collections::HashMap<&str, &TensorMeta> = mod_tensors
        .iter()
    ...
    // Pass 1: compute all element-wise diffs and find the global max.
    struct TensorDiff<'a> {
        orig: &'a TensorMeta,
        diffs: Vec<f32>,
    }

    let mut tensor_diffs: Vec<TensorDiff> = Vec::with_capacity(orig_tensors.len());
    ...
    for orig_t in &orig_tensors {
        let mod_t = match mod_map.get(orig_t.name.as_str()) {
            ...
            None => {
                eprintln!(
                    "warning: safetensors diff: tensor '{}' not found in modified file — skipping",
                    orig_t.name
                );
                continue;
            }
        ...
        if orig_t.dtype != mod_t.dtype || orig_t.shape != mod_t.shape {
            eprintln!(
                "warning: safetensors diff: tensor '{}' dtype/shape mismatch — skipping",
                orig_t.name
            );
            continue;
        }

        let orig_bytes = &original[orig_t.file_start as usize..orig_t.file_end as usize];
        let mod_bytes = &modified[mod_t.file_start as usize..mod_t.file_end as usize];

        let orig_vals = orig_t.dtype.decode_to_f32(orig_bytes);
        let mod_vals = mod_t.dtype.decode_to_f32(mod_bytes);

        let diffs: Vec<f32> = orig_vals
            .iter()
            .zip(mod_vals.iter())
            .map(|(&a, &b)| (a - b).abs())
            .collect();
        ...
        for &d in &diffs {
            if d.is_finite() && d > global_max {
                global_max = d;
            }
        }

        tensor_diffs.push(TensorDiff { orig: orig_t, diffs });
    }

    // Pass 2: normalize with a log scale and write into output buffer.
    //
    // log1p(diff) / log1p(global_max) maps [0, global_max] → [0, 1] on a
    // logarithmic curve, then scales to [0, 255].  This compresses the high
    // end and expands the low end so small differences (which dominate in
    // typical instruction-tuning diffs) become visually distinguishable.
    let log_max = if global_max > 0.0 { (global_max as f64 + 1.0).ln() } else { 1.0 };
    let elem_size = |dtype: Dtype| dtype.element_size();

    for td in &tensor_diffs {
        let t = td.orig;
        let es = elem_size(t.dtype);
        let file_start = t.file_start as usize;
        let file_end = t.file_end as usize;

        // Write one u8 value per *element*, repeated across its byte span.
        for (i, &diff) in td.diffs.iter().enumerate() {
            ...
            let start = file_start + i * es;
            let end = (start + es).min(file_end);
            if start < file_end {
                output[start..end].fill(byte_val);
            }
        }
    }
